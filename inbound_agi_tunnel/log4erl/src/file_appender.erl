...
%%======================================
init({Dir, Fname, {Type, Max}, Rot, Suf, Level} = Conf) when is_tuple(Conf) ->
    ...
    ?LOG2("To parse format with default ~p~n",[?DEFAULT_FORMAT]),
    {ok, Format} = log_formatter:parse(?DEFAULT_FORMAT),
